...
§
Hermes-agent has a custom patch applied as a git commit on top of upstream: `custom-provider-enhancements.patch` at `~/.hermes/custom-provider-enhancements.patch`. The patch adds `custom:<name>` provider resolution and live `/models` probing. To update: `git reset --hard HEAD~1 && git pull --ff-only && git am < ~/.hermes/custom-provider-enhancements.patch`. If am fails: `git am --abort` then regenerate from skill. Skill: hermes-agent-patch.